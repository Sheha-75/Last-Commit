...
import {
  ArrowLeftRight, Users, ShieldAlert, Ban, UserX, BrainCircuit,
  TrendingUp, TrendingDown, Activity, Globe2, Zap, Cpu, Clock,
} from 'lucide-react';
import { PageHeader } from '@/layouts/SocLayout';
...
import type { Transaction, ToastNotification } from '@/types';
...
const kpiDefs = [
  ...
  { id: 'activeUsers', label: 'Active Users', icon: Users, accent: 'text-blue-400', bg: 'bg-blue-500/15', trend: 4.2 },
  ...
  { id: 'highRiskAccounts', label: 'High Risk Accounts', icon: UserX, accent: 'text-orange-400', bg: 'bg-orange-500/15', trend: 3.7 },
  { id: 'aiRiskScore', label: 'AI Risk Score', icon: BrainCircuit, accent: 'text-cyan-400', bg: 'bg-cyan-500/15', trend: 1.8 },
];
...
function KpiCard({ label, value, icon: Icon, accent, bg, trend, index }: typeof kpiDefs[number] & { value: number; index: number }) {
  ...
  const isPct = label === 'AI Risk Score';
  ...
  return (
    ...
    >
      <Card glow className="p-5 h-full">
        ...
        <motion.div
          className="mt-3 h-1 rounded-full bg-soc-border overflow-hidden"
        >
          <motion.div
            initial={{ width: 0 }}
...
export default function Dashboard({ pushToast }: { pushToast: (t: Omit<ToastNotification, 'id' | 'timestamp'>) => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const [k, tl, lg, rd, t] = await Promise.all([
        fraudService.getKpis(),
        fraudService.getTimeline(),
        fraudService.getLoginAttempts(),
        fraudService.getRiskDistribution(),
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        {loading || !kpis
          ...
          : kpiDefs.map((k, i) => <KpiCard key={k.id} {...k} value={kpis[k.id]} index={i} />)}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mt-6">
        <Card className="xl:col-span-2 overflow-hidden">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Globe2 className="w-4 h-4 text-soc-accent" /> Global Threat Map</CardTitle>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Activity className="w-4 h-4 text-soc-accent" /> Animated Risk Gauge</CardTitle>
          </CardHeader>
          <CardBody>
            {loading ? <Skeleton className="h-[220px] w-full" /> : <RiskGauge value={73} />}
          </CardBody>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Zap className="w-4 h-4 text-soc-accent" /> Fraud Probability Speedometer</CardTitle>
          </CardHeader>
          <CardBody>
            {loading ? <Skeleton className="h-[220px] w-full" /> : <FraudSpeedometer value={97} />}
          </CardBody>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
        ...
        <Card>
          <CardHeader>
            ...
            <Badge variant="high">elevated</Badge>
          </CardHeader>
          <CardBody>
            {loading ? (
              <Skeleton className="h-[240px] w-full" />
      ...
      {/* Incident timeline + threat feed */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-6">
        <Card className="lg:col-span-1">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Clock className="w-4 h-4 text-soc-accent" /> Incident Timeline</CardTitle>
            <Badge variant="critical">{recentIncidents.length}</Badge>
          </CardHeader>
          <CardBody className="pt-0">
            <div className="relative pl-6 mt-4">
              <div className="absolute left-2 top-0 bottom-0 w-px bg-gradient-to-b from-soc-accent via-soc-border to-transparent" />
        ...
        <Card className="lg:col-span-2 overflow-hidden">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><ShieldAlert className="w-4 h-4 text-soc-accent" /> Threat Feed</CardTitle>
            <Badge variant="neutral">{topTx.length} flagged</Badge>
          </CardHeader>
                  ...
                  {topTx.map((t, i) => (
                    ...
                    >
                      ...
                      <td className="py-2.5 pr-3 hidden md:table-cell">
                        {t.vpn ? <Badge variant="high">Yes</Badge> : <Badge variant="low">No</Badge>}
                      </td>
  ...
  return (
    <Card className="relative overflow-hidden">
      ...
      <CardHeader className="relative">
        <CardTitle className="flex items-center gap-2"><Cpu className="w-4 h-4 text-soc-accent" /> AI Explanation</CardTitle>
          ...
          <>
            ...
            <div className="mt-4">
              <div className="flex justify-between text-[10px] text-slate-500 mb-1">
                <span>Confidence</span><span>97%</span>
              </div>
