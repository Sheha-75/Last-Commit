...
import {
  ...
  BrainCircuit, FlaskConical, Settings, Bell, Search, LogOut, Menu, X, ChevronRight,
} from 'lucide-react';
...
export function SocLayout({ notifications, onOpenNotifications, pushToast: _pushToast }: Props) {
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-5 h-16 border-b border-soc-border shrink-0">
        ...
        <div className="leading-tight">
          <p className="text-sm font-bold text-white">SentinelShield<span className="text-soc-accent"> AI</span></p>
          <p className="text-[10px] text-slate-500 font-mono">SOC v4.2 · LIVE</p>
        </div>
      </div>
        ...
        {nav.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <motion.span layoutId="nav-active" className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 rounded-r-full bg-soc-accent" />
                )}
  ...
  return (
    <div className="relative min-h-screen bg-soc-bg">
      ...
      <div className="lg:pl-64 relative z-10">
        ...
        <header className="sticky top-0 z-20 h-16 glass border-b border-soc-border flex items-center gap-4 px-4 sm:px-6">
          ...
          <button
            ...
            className="relative p-2 rounded-xl glass-soft hover:border-soc-accent/40 transition-colors"
          >
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 p-1 pr-2 rounded-xl glass-soft hover:border-soc-accent/40 transition-colors"
            >
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-soc-primary to-soc-accent flex items-center justify-center text-sm font-bold text-white">
                {user?.name?.[0] ?? 'A'}
              </div>
              <div className="hidden sm:block text-left leading-tight">
                <p className="text-xs font-semibold text-white">{user?.name ?? 'Analyst'}</p>
                <p className="text-[10px] text-slate-500">SOC Analyst</p>
              </div>
            </button>
            <AnimatePresence>
              {profileOpen && (
      ...
      {/* Mobile close button for sidebar (inside) */}
      {sidebarOpen && (
